...
VERDICT_COLORS = {"Fake":"#ef4444","Authentic":"#10b981","Misleading":"#f59e0b"}
...
@st.cache_resource
def train_models(df):
    les = {c: LabelEncoder() for c in
           ["News_Verdict","Channel_Watched","Anchor_Name","News_Category","Consumption_Frequency"]}
    df2 = df.copy()
    for c, le in les.items():
        df2[c+"_enc"] = le.fit_transform(df2[c])

    feats = ["Channel_Watched_enc","Anchor_Name_enc","News_Category_enc",
             "Consumption_Frequency_enc","TRP_Score","Sensationalism_Score",
    ...
    Xtr, Xte, ytr, yte = train_test_split(X_s, y_cls, test_size=0.2, random_state=42)
    clf = LogisticRegression(max_iter=300, random_state=42, C=1.5)
    clf.fit(Xtr, ytr)
    acc = accuracy_score(yte, clf.predict(Xte))

    reg_s = Ridge(alpha=1.0); reg_s.fit(X_s, y_sent)
    reg_t = Ridge(alpha=1.0); reg_t.fit(X_s, y_trust)
...
with st.sidebar:
    ...
    sel_ch  = st.multiselect("Channel",   df["Channel_Watched"].unique().tolist(),
                              default=df["Channel_Watched"].unique().tolist())
    sel_an  = st.multiselect("Anchor",    df["Anchor_Name"].unique().tolist(),
                              default=df["Anchor_Name"].unique().tolist())
    sel_yr  = st.slider("Year Range", 2022, 2025, (2022, 2025))
    sel_cat = st.multiselect("Category",  df["News_Category"].unique().tolist(),
...
if page == "🏠 Media Pulse":
    ...
    with r1c1:
        ...
        fig = px.pie(vc, values="count", names="News_Verdict",
                     color="News_Verdict", color_discrete_map=VERDICT_COLORS,
                     hole=0.4)
    ...
    with r1c2:
        ...
        ch_fake = (fdf.groupby("Channel_Watched")
                   .apply(lambda x: (x["News_Verdict"]=="Fake").mean()*100)
                   .reset_index(name="Fake_Rate")
                   .sort_values("Fake_Rate"))
        fig2 = px.bar(ch_fake, x="Fake_Rate", y="Channel_Watched", orientation="h",
    ...
    with r2c1:
        ...
        m = (fdf.groupby("Month_dt")["Sentiment_Score"].mean()
               .reset_index().sort_values("Month_dt"))
        m["lbl"] = m["Month_dt"].dt.strftime("%b %y")
        fig3 = px.line(m, x="lbl", y="Sentiment_Score",
                       labels={"Sentiment_Score":"Avg Sentiment","lbl":""})
        fig3.add_hline(y=0, line_dash="dash", line_color="#6b7280",
                       annotation_text="Neutral", annotation_font_color="#9ca3af")
        fig3.update_traces(line_color="#6c63ff", line_width=2.5, fill="tozeroy",
    ...
    with r2c2:
        ...
        anc = (fdf.groupby("Anchor_Name")[["Fake_Units_Consumed","Authentic_Units_Consumed"]]
                  .mean().reset_index()
                  .sort_values("Fake_Units_Consumed", ascending=False))
        fig4 = go.Figure([
            go.Bar(name="Fake",      x=anc["Anchor_Name"], y=anc["Fake_Units_Consumed"],
                   marker_color="#ef4444"),
            go.Bar(name="Authentic", x=anc["Anchor_Name"], y=anc["Authentic_Units_Consumed"],
                   marker_color="#10b981")
        ])
        fig4.update_layout(**PLT, barmode="group", height=300,
                           legend=dict(orientation="h", y=1.1, x=0.5, xanchor="center"),
                           xaxis=dict(tickangle=15))
        st.plotly_chart(fig4, use_container_width=True)

    # PyDeck — Regional Fake News Map
    st.markdown("---")
    st.markdown('<div class="sec-head">🗺️ Regional Fake News Heat Map (India)</div>',
                unsafe_allow_html=True)

    reg_stats = (fdf.groupby("Region")
                    .agg(Fake_Rate=("News_Verdict", lambda x: (x=="Fake").mean()*100),
    ...
    reg_stats["radius"] = reg_stats["Fake_Rate"] * 9000
    reg_stats["r"] = 239; reg_stats["g"] = 68; reg_stats["b"] = 68
    reg_stats["opacity"] = (reg_stats["Fake_Rate"] / reg_stats["Fake_Rate"].max() * 180).astype(int)
    ...
    tooltip = {"html": "<b>{Region}</b><br/>Fake Rate: {Fake_Rate:.1f}%<br/>Viewers: {Viewers}",
               "style": {"background":"#1e2130","color":"white","font-family":"sans-serif","padding":"8px"}}
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with d2:
        st.markdown('<div class="sec-head">📡 TRP vs Fake News — Channel Bubble</div>',
                    unsafe_allow_html=True)
        bs = fdf.groupby("Channel_Watched").agg(
            Avg_TRP=("TRP_Score","mean"),
            Fake_Rate=("News_Verdict", lambda x: (x=="Fake").mean()*100),
            Avg_Sent=("Sentiment_Score","mean"),
            Viewers=("Viewer_ID","count")
        ).reset_index()
        fig2 = px.scatter(bs, x="Avg_TRP", y="Fake_Rate",
                          size="Viewers", color="Avg_Sent",
                          text="Channel_Watched",
                          color_continuous_scale="RdYlGn_r",
                          labels={"Avg_TRP":"Avg TRP","Fake_Rate":"Fake Rate (%)","Avg_Sent":"Sentiment"})
    ...
    d3, d4 = st.columns(2)
    with d3:
        st.markdown('<div class="sec-head">😨 Sentiment by Category × Verdict</div>',
                    unsafe_allow_html=True)
        cs = fdf.groupby(["News_Category","News_Verdict"])["Sentiment_Score"].mean().reset_index()
        fig3 = px.bar(cs, x="News_Category", y="Sentiment_Score", color="News_Verdict",
                      barmode="group", color_discrete_map=VERDICT_COLORS,
                      labels={"Sentiment_Score":"Avg Sentiment","News_Category":""})
        fig3.update_layout(**PLT, height=310,
                           legend=dict(orientation="h", y=1.1, x=0.5, xanchor="center"))
        st.plotly_chart(fig3, use_container_width=True)

    with d4:
        st.markdown('<div class="sec-head">🧑‍🤝‍🧑 Knowledge Gap by Age Group</div>',
                    unsafe_allow_html=True)
        ag = fdf.groupby("Age_Group").agg(
            Fake_Pct=("News_Verdict", lambda x: (x=="Fake").mean()*100),
            Avg_Know=("Knowledge_Accuracy_Pct","mean")
        ).reset_index()
        fig4 = px.scatter(ag, x="Fake_Pct", y="Avg_Know",
                          text="Age_Group", size="Fake_Pct",
                          color="Avg_Know", color_continuous_scale="RdYlGn",
    ...
    # Pydeck — Authentic news viewer flow arcs between regions
    st.markdown("---")
    st.markdown('<div class="sec-head">🗺️ Authentic vs Fake Viewer Distribution (Regional)</div>',
                unsafe_allow_html=True)
    reg2 = fdf.groupby("Region").agg(
    ...
    fake_layer = pdk.Layer("ColumnLayer", data=reg2,
        get_position=["lon","lat"], get_elevation="Fake_Viewers",
        elevation_scale=10, radius=60000,
        get_fill_color=[239,68,68,180], pickable=True, auto_highlight=True)
    auth_layer = pdk.Layer("ColumnLayer", data=reg2,
        get_position=["lon","lon"],
        get_position=["lon","lat"],
        get_elevation="Auth_Viewers",
        elevation_scale=10, radius=40000,
        get_fill_color=[16,185,129,180], pickable=True, auto_highlight=True)
    view2 = pdk.ViewState(latitude=22.5, longitude=80.0, zoom=3.8, pitch=45, bearing=10)
    tooltip2 = {"html": "<b>{city} ({Region})</b><br/>🔴 Fake Viewers: {Fake_Viewers}<br/>🟢 Auth Viewers: {Auth_Viewers}<br/>Trust: {Avg_Trust:.1f}",
                "style": {"background":"#1e2130","color":"white","padding":"8px"}}
    st.pydeck_chart(pdk.Deck(
        layers=[fake_layer, auth_layer],
        initial_view_state=view2, tooltip=tooltip2,
        map_style="mapbox://styles/mapbox/dark-v10"
    ))
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with v3:
        ...
        go_btn = st.button("🔮 Predict Now", use_container_width=True, type="primary")
    ...
    with pc1:
        ...
        fig_p = px.bar(prob_df, x="Verdict", y="Probability",
                       ...
                       labels={"Probability":"Probability (%)"}, text_auto=".1f")
    ...
    with pc2:
        ...
        fig_g = go.Figure(go.Indicator(
            ...
            gauge={"axis":{"range":[-5,5],"tickcolor":"#e2e8f0"},
                   "bar":{"color":s_color}, "bgcolor":"#1e2130",
                   "steps":[{"range":[-5,-2],"color":"#7f1d1d"},
    ...
    if pred_v == "Fake":
        box("box-red",    "🚨 <b>High Risk:</b> This viewer is primarily consuming fake news — emotional damage and knowledge erosion are likely.")
    elif pred_v == "Misleading":
        box("box-yellow", "⚠️ <b>Moderate Risk:</b> Half-truths are distorting this viewer's understanding. Partial misinformation is equally damaging over time.")
    else:
        box("box-green",  "✅ <b>Healthy Media Diet:</b> This viewer primarily consumes authentic news — trust and knowledge scores are above average.")

    # Pydeck — highlight viewer's region on map
    st.markdown("---")
    st.markdown('<div class="sec-head">🗺️ Viewer's Predicted Emotional Zone — India Map</div>',
                unsafe_allow_html=True)
    all_reg = pd.DataFrame([
        {"Region":r,"lat":v["lat"],"lon":v["lon"],"city":v["city"],
         "Pred_Sentiment":pred_s,"r":16 if pred_s>0 else 239,
         "g":185 if pred_s>0 else 68,"b":129 if pred_s>0 else 68,
         "radius":abs(pred_s)*50000+120000}
        for r,v in REGION_COORDS.items()
    ])
    pulse_layer = pdk.Layer("ScatterplotLayer", data=all_reg,
        get_position=["lon","lat"], get_radius="radius",
        get_fill_color=["r","g","b",120], pickable=True,
        stroked=True, line_width_min_pixels=2,
        get_line_color=[255,255,255,80])
    txt_layer = pdk.Layer("TextLayer", data=all_reg,
        get_position=["lon","lat"], get_text="city",
    ...
    tooltip3 = {"html": f"<b>{{city}}</b><br/>Predicted Sentiment: <b>{pred_s:+.2f}</b> ({sentiment_label})",
                "style": {"background":"#1e2130","color":"white","padding":"8px"}}
    st.pydeck_chart(pdk.Deck(layers=[pulse_layer, txt_layer],
                             initial_view_state=view3, tooltip=tooltip3,
                             map_style="mapbox://styles/mapbox/dark-v10"))
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with l1:
        ...
        ch_lb = df.groupby("Channel_Watched").agg(
            Trust=("Trust_Score","mean"),
            Authentic_Rate=("News_Verdict", lambda x:(x=="Authentic").mean()*100),
            Knowledge=("Knowledge_Accuracy_Pct","mean")
        ).round(1).reset_index().sort_values("Trust", ascending=False).reset_index(drop=True)
        ch_lb.index += 1
        medals = {1:"🥇",2:"🥈",3:"🥉"}
        ch_lb.insert(0,"#", [medals.get(i,"") for i in ch_lb.index])
        st.dataframe(ch_lb.rename(columns={"Channel_Watched":"Channel",
                                           "Trust":"Trust /10",
    ...
    with l2:
        ...
        an_int = df.groupby("Anchor_Name").agg(
            Trust=("Trust_Score","mean"),
            Sentiment=("Sentiment_Score","mean"),
            Fake_Rate=("News_Verdict", lambda x:(x=="Fake").mean()*100)
        ).round(2).reset_index()
        an_int["Integrity"] = (an_int["Trust"]*4 + an_int["Sentiment"]*6 - an_int["Fake_Rate"]*0.3).round(1)
        an_int = an_int.sort_values("Integrity", ascending=False)
        fig_i = px.bar(an_int, x="Anchor_Name", y="Integrity",
                       color="Integrity", color_continuous_scale="RdYlGn",
                       labels={"Anchor_Name":"","Integrity":"Integrity Score"},
                       text="Integrity")
        fig_i.update_layout(**PLT, height=290, coloraxis_showscale=False,
                            xaxis=dict(tickangle=15))
    ...
    if fake_r > 0.40:
        box("box-red",    f"🔴 <b>Switch Channel:</b> {my_ch} has a {fake_r*100:.1f}% fake news rate. Try <b>{best_overall_ch}</b> instead.")
    if sens_v > 7:
        box("box-yellow", f"🟡 <b>Sensationalism Trap:</b> {my_ch} scores {sens_v:.1f}/10 — headlines are engineered to trigger fear. Pause before reacting.")
    if my_freq == "Daily" and fake_r > 0.35:
        box("box-red",    "🔴 <b>Reduce Frequency:</b> Daily consumption of high-fake channels compounds emotional damage. Switch to weekly curated reading.")
    if my_age in ["18-25","60+"]:
        box("box-yellow", f"🟡 <b>Vulnerability Alert:</b> The {my_age} age group is statistically more susceptible to fake news. Always cross-check before sharing.")
    box("box-green", f"✅ <b>Best Anchor Recommendation:</b> <b>{best_overall_an}</b> scores highest on integrity — prioritise their coverage.")
    box("box-green", "✅ <b>The 3-Source Rule:</b> Reading across 3+ channels with different editorial stances reduces fake news exposure by ~62%.")
    box("box-green", "✅ <b>The 3-Second Rule:</b> If a headline makes you angry within 3 seconds of reading — it's a sensationalism trigger, not journalism.")

    # Sentiment Recovery Simulator
    st.markdown("---")
    ...
    fig_sim = go.Figure()
    fig_sim.add_trace(go.Bar(name="Before", x=["Sentiment","Knowledge %"],
                             y=[round(cs,2), round(ck,1)], marker_color="#ef4444"))
    fig_sim.add_trace(go.Bar(name="After",  x=["Sentiment","Knowledge %"],
                             y=[round(ps,2), round(pk,1)], marker_color="#10b981"))
    fig_sim.update_layout(**PLT, barmode="group", height=280,
                          legend=dict(orientation="h", y=1.1, x=0.5, xanchor="center"))
    st.plotly_chart(fig_sim, use_container_width=True)

    ds, dk = ps-cs, pk-ck
    if ds > 0:
        box("box-green", f"📈 This media diet change improves your sentiment by <b>{ds:+.2f}</b> and knowledge by <b>{dk:+.1f}%</b>.")
    else:
        box("box-red",   f"📉 This pattern worsens your sentiment by <b>{ds:.2f}</b>. Increase authentic news consumption.")
